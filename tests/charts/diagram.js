...
async function initDataSet(xParam, yParam, browser, vis, nodes) {
  ...
  const filteredData = rawData.filter(item => 
    item.Browser === browser && item.Visualisation === vis && item.Nodes === nodes
  );
...
async function generateChart(){
  const chartDataSets = await initDataSet("Threads", "Execution time","Chrome",true,100);
...
function assignColors(implementations) {
  const colors = [
    ...
    "#FF9F40", "#C9CBCF", "#7E7F9A", "#FFD700", "#DC143C", 
    "#00FFFF", "#00008B", "#008B8B", "#B8860B", "#006400"
  ];
